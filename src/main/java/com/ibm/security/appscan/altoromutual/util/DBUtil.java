...
//				"(800002, 'AAPL', 200)");
	 ...
	 * First --> Insert
	 */
	public static void updateHoldingTable(long accountIDNumber, String stockSymbol, int holdingAmount) throws SQLException {
		Connection connection = getConnection();
		Statement statement = connection.createStatement();
	...
	public static void updateHoldingAmount(long accountIDNumber, String stockSymbol, int newAmount) throws SQLException {
		...
		statement.executeUpdate("UPDATE HOLDINGS SET HOLDINGAMOUNT = "+ newAmount +" WHERE ACCOUNTID = "+ accountIDNumber +" AND STOCKSYMBOL = '"+ stockSymbol +"' ");
	}
	...
	public static String tradeStock(String tradeAccountID, String tradeType, int tradeAmount, double tradePrice, String stockSymbol) {
		...
		try {
			...
			if(tradeType.equals("buy")) {
				...
				// update trade table
				addTradeRecord(tradeAccount.getAccountId(), date, tradeType, stockSymbol, tradeAmount, tradePrice);

				// update holding table
				int currentHolding = getStockShare(tradeAccount.getAccountId(), stockSymbol);
				if (currentHolding < 0) {
					updateHoldingTable(tradeAccount.getAccountId(), stockSymbol, tradeAmount);
				} else {
